...
function normalizeVideo(payload: unknown) {
  ...
  if (!id || !title || !description || !url) return null;
  return { id, title, description, url, thumbnail, views, comments };
}

function normalizeOwnerUser(payload: unknown) {
  if (!payload || typeof payload !== "object") return null;
  const item = payload as Record<string, unknown>;
...
app.post("/video-poster", async (req, res) => {
  try {
    const body = req.body as { dataUrl?: string; videoId?: string; reelId?: string };
