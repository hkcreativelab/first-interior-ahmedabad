...
function OwnerPortalPage() {
  ...
  return (
    <div className="relative min-h-screen bg-background text-ink">
      <Nav />
      <main className="relative mx-auto flex min-h-screen items-center justify-center px-6 py-24">
        <div className="absolute inset-0 bg-ink/10" />
        <div className="relative z-10 w-full max-w-6xl rounded-[2rem] border border-border bg-cream/95 p-6 shadow-[0_35px_120px_rgba(15,23,42,0.18)] backdrop-blur-xl sm:p-10">
          <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr]">
                <div className="rounded-[1.75rem] border border-border bg-forest px-8 py-10 text-cream shadow-[0_20px_80px_rgba(15,23,42,0.18)]">
              <p className="text-xs uppercase tracking-[0.35em] text-sand/80">Owner portal</p>
              <h1 className="mt-6 text-4xl font-semibold leading-tight">Secure admin access</h1>
              ...
              <div className="mt-8 rounded-[1.5rem] bg-ink/10 p-6 text-sm text-cream/80">
                <p className="font-semibold text-cream">Portal features</p>
            ...
            <div className="space-y-8">
              {!isAuthenticated ? (
                <div className="rounded-[1.75rem] border border-border bg-cream p-8 shadow-sm sm:p-10">
                  <div className="flex items-center gap-4 rounded-3xl bg-cream/80 p-5 text-ink">
                  ...
                  <form onSubmit={handleLogin} className="mt-8 space-y-5">
                    <label className="block text-sm font-medium text-ink">
                      ...
                      <input
                        ...
                        className="mt-3 w-full rounded-3xl border border-border bg-cream px-4 py-3 text-sm outline-none transition focus:border-forest"
                        placeholder="Enter passcode"
                      />
                    </label>
                    {message ? <p className="text-sm text-ink">{message}</p> : null}
                    <button className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-forest px-6 py-3 text-sm font-semibold text-cream transition hover:bg-ink">
                      Unlock portal
                    </button>
                  </form>
                </div>
                ...
                <div className="grid gap-8">
                  <div className="rounded-[1.75rem] border border-border bg-cream p-8 shadow-sm sm:p-10">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <h2 className="text-3xl font-semibold text-ink">Add new reel</h2>
                      </div>
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="rounded-full border border-border bg-cream/80 px-4 py-2 text-sm font-semibold text-ink transition hover:bg-cream/90"
                      >
                        Log out
                      </button>
                    </div>

                    <form onSubmit={handleAddReel} className="grid gap-5 mt-8">
                      ...
                      <label className="block text-sm text-ink">
                        ...
                        <textarea
                          ...
                          className="mt-3 w-full min-h-[120px] rounded-3xl border border-border bg-cream px-4 py-3 text-sm outline-none transition focus:border-forest"
                      ...
                      <label className="block text-sm text-ink">
                        ...
                        <input
                          ...
                          className="mt-3 w-full rounded-3xl border border-border bg-cream px-4 py-3 text-sm outline-none transition file:rounded-full file:border-0 file:bg-forest file:px-4 file:py-2 file:text-cream file:font-semibold file:transition hover:file:bg-ink"
                        />
                      </label>
                      ...
                      <button
                        type="submit"
                        className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-forest px-6 py-3 text-sm font-semibold text-cream transition hover:bg-ink"
                      >
                        Add reel
                      </button>
                    </form>
                  ...
                  <div className="rounded-[1.75rem] border border-border bg-cream p-8 shadow-sm sm:p-10">
                    <div className="space-y-6">
                      <div className="rounded-3xl bg-cream/80 p-5 text-ink">
                        <p className="font-semibold text-ink">Live reel management</p>
                      ...
                      <div className="rounded-3xl border border-border bg-cream p-5">
                        ...
                        <p className="mt-3 text-3xl font-semibold text-ink">{reels.length}</p>
                      </div>
                        ...
                        {hasReels ? (
                          <div className="mt-5 space-y-4">
                            {reels.map((reel) => (
                              <div
                                key={reel.id}
                                className="flex items-center justify-between rounded-3xl border border-border bg-cream p-4"
                              >
                                <div>
                                  <p className="font-semibold text-ink">{reel.title}</p>
                                  <p className="text-sm text-ink/70 truncate">{reel.url}</p>
                                </div>
                                <button
                                  type="button"
                      ...
                      <div className="space-y-4">
                        <div>
                          <p className="text-sm font-semibold text-ink">Change passcode</p>
                          <p className="text-sm text-ink/70">
                            Update the password for owner access.
                          </p>
                        </div>

                        <form onSubmit={handleChangePasscode} className="grid gap-4">
                          <input
                            type="password"
                            value={currentPasscode}
                            onChange={(event) => setCurrentPasscode(event.target.value)}
                            placeholder="Current passcode"
                            className="w-full rounded-3xl border border-border bg-cream px-4 py-3 text-sm outline-none transition focus:border-forest"
                          />
                          <input
                            type="password"
                            value={newPasscode}
